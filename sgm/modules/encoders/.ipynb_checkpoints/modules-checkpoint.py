...
class FrozenT5Embedder(AbstractEmbModel):
    ...
    def __init__(
        ...
        super().__init__()
        if model_dir is not "google/t5-v1_1-base":
...
class FrozenGLM4Embedder(AbstractEmbModel):
    """
    Uses the GLM-4 transformer encoder for text. Is character-aware.
    """
    def __init__(
        self,
        ...
        if model_dir != "THUDM/glm-4-9b":
            self.tokenizer = AutoTokenizer.from_pretrained(model_dir, trust_remote_code=True)
            self.transformer =  AutoModel.from_pretrained(model_dir, trust_remote_code=True)
        else:
            self.tokenizer = AutoTokenizer.from_pretrained(model_dir, cache_dir=cache_dir, trust_remote_code=True)
            self.transformer = AutoModel.from_pretrained(model_dir, cache_dir=cache_dir, trust_remote_code=True)
